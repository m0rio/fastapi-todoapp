...
def login(request: Request, response: Response, user: UserBody, csrf_protect: CsrfProtect = Depends()):
    csrf_token = csrf_protect.get_csrf_from_headers(request.headers)
    csrf_protect.validate_csrf(csrf_token)
    user = jsonable_encoder(user)
    token = db_login(user)
...
def get_user_reflesh_jwt(reqest: Request, response: Response):
    # subject = email
    new_token, subject = auth.verify_update_jwt(reqest)
    response.set_cookie(
